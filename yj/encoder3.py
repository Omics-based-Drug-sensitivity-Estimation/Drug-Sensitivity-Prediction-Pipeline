...
class ChemBERTaOmicsDataset(Dataset):
    ...
    def __init__(
        ...
        *,
        ...
        # CID도 가져오기 (optional)
        if 'CID' in smiles_df.columns:
            smiles_df = smiles_df[['drug', 'SMILES', 'CID']]
        else:
            smiles_df['CID'] = None  # 없으면 None
        ...
        rows: List[Tuple[str, int, float, str, str, str]] = []  # (SMILES, cell_idx, IC50, cell_line, drug, CID)
        missing: set[str] = set()
        for _, r in sens.iterrows():
            ...
            if cl not in cell2idx:
                ...
                continue
            rows.append((
                r["SMILES"], cell2idx[cl], r["IC50"], cl, r["drug"], r["CID"]
            ))
        if missing:
    ...
    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor | str]:
        smiles, cell_idx, ic50, cell_line, drug, cid = self.rows[idx]
        return {
...
def chemberta_collate(batch):
    output = {}
    # Tensor 타입만 stack, 나머지는 list로 묶기
    for k in batch[0]:
