...
import torch
from data.TripleOmics_ChemBERT_Dataset import ChemBERTaOmicsDataset, chemberta_collate
...
def main(
    ...
    for fold in range(n_folds):
        ...
        model =  ModelClass(params).to(get_device())
        model.train()
...
def training(model, device, epoch, fold, train_loader, optimizer, params, t, max_value, min_value):
    ...
    for ind, batch in enumerate(progress_bar):
        ...
        torch.nn.utils.clip_grad_norm_(model.parameters(), 5e-7)
        optimizer.step()
...
if __name__ == "__main__":
    ...
    model_version = "v1"  # Choose from "v1", "v2", "v3"
    ...
    params = {
        'fold': 1, 'optimizer': "adam", 'smiles_padding_length': 128, 'smiles_embedding_size': 384,
        'number_of_pathways': 619, "train_backbone": False, 'smiles_attention_size': 256,
        'gene_attention_size': 1, 'molecule_temperature': 1.0, 'gene_temperature': 1.0,
        'molecule_gep_heads': [2], 'molecule_cnv_heads': [2], 'molecule_mut_heads': [2],
        'gene_heads': [1], 'cnv_heads': [1], 'mut_heads': [1], 'n_heads': 2, 'num_layers': 4,
        'omics_dense_size': 256, 'stacked_dense_hidden_sizes': [1024, 512], 'dropout': 0.5,
        'temperature': 1.0, 'activation_fn': 'relu', 'batch_norm': True,
        'drug_sensitivity_processing_parameters': {'parameters': {"min": -8.658382, "max": 13.107465}},
        'loss_fn': 'mse', 'model_version': model_version
    }
    training_name = f'PASO_{model_version}_train'

    # Load model
    ModelClass = MODEL_FACTORY[model_version]
    model = ModelClass(params).to(get_device())
    # Run the training
    main(
        drug_sensitivity_filepath,
